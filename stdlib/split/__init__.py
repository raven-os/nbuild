...
All splitters are functions that try split the output of a build into one or multiple packages.
They approach the problem differently, letting the maintainer chose the one that fits most the package
he is building.

They all share the same proprieties which are essentials to ensure they can be used interchangeably:
  ...
  * The :py:func:`~stdlib.package.PackageID.short_name` used as a key is guaranteed to match the one's of the package matching that key.
